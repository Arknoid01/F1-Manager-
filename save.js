...
const Save = {
  ...
  recordRaceResults(raceState, results) {
    ...
    // Les pilotes progressent naturellement selon leurs résultats
    // Gains très faibles — la vraie progression se fait sur 2-3 saisons
    ...
    results.forEach(r => {
      ...
      // Pas de progression sur abandon
      if (isDnf) return;

      // Facteur âge : jeunes progressent plus, vieux moins
      // < 23 ans : x1.2 | 23-28 : x1.0 | 29-33 : x0.6 | 34+ : x0.2 (légère régression possible)
      const ageFactor = age < 23 ? 1.2 : age < 29 ? 1.0 : age < 34 ? 0.6 : 0.2;

      // Facteur résultat : meilleures positions = plus d'apprentissage
      ...
      // Stat à améliorer selon le résultat
      // Bon résultat → améliore pace / consistency
      // Mauvais résultat → améliore defending / overtaking (apprentissage en difficulté)
      ...
      // Plafond potentiel strict — jamais au-delà
      if (current >= pot) return;
