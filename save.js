...
const Save = {
  ...
  recordRaceResults(raceState, results) {
    ...
    // Versement 1 : 40% sponsors au démarrage (course 1)
    if (raceIdx === 0) {
      if (totalSponsorAnnual > 0) {
        sponsorBonus = Math.round(totalSponsorAnnual * 0.40 * 10) / 10;
        if (save.news) save.news.unshift({ icon:'💰', category:'finance', date:new Date().toISOString(),
          title:'Versement sponsors — Début de saison',
          text:`40% des contrats sponsors versés : +${sponsorBonus}M€` });
      }
      // Avance Concorde 30% dès le début — les équipes reçoivent une avance FOM
      const concordeRevs = F1Data.concordeRevenues || [120,100,85,72,62,54,46,38,30,22,15];
      const teamRankEst  = [...F1Data.teams].sort((a,b)=>(save.teamStandings?.[b.id]||0)-(save.teamStandings?.[a.id]||0)).findIndex(t=>t.id===playerTeamId)+1 || 8;
      const concordeAdv  = Math.round((concordeRevs[Math.min(teamRankEst-1, concordeRevs.length-1)]||15) * 0.30);
      save.budget = Math.round(((save.budget||0) + concordeAdv) * 10) / 10;
      if (save.news) save.news.unshift({ icon:'💵', category:'finance', date:new Date().toISOString(),
        title:'Avance revenus FOM',
        text:`30% des revenus Concorde versés en avance : +${concordeAdv}M€` });
    }
